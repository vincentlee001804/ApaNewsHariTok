...
def get_todays_news_digest_for_user(
    ...
    *,
    ...
    from src.ai.summarizer import summarize_digest, summarize_digest_overview
    ...
    def digest_bullet_points(raw_digest: str, *, max_points: int = 5) -> list[str]:
        text = strip_markdown_artifacts_for_plain_text(raw_digest or "").strip()
        if not text:
            return []
        ...
        for p in points[:max_points]:
            clipped = clip_plain_text_to_word_limit(p, 28)
            if clipped:
    ...
    if not candidates:
        ...
        for it in display_items:
            ...
            if it.link:
                source_name = escape_html((getattr(it, "source", None) or "Source").strip())
                lines.append(f'<a href="{it.link}">{source_name}</a>')
            lines.append("")
        lines.append("Summary generated by AI, please verify the accuracy.")
        return "\n".join(lines).strip()
    ...
    chosen_clusters: list[tuple[NewsArticle, list[NewsArticle]]] = (
        _cluster_ranked_articles_cross_source(ranked_articles, max_items=max_articles)
    )
    chosen: List[NewsArticle] = [primary for primary, _members in chosen_clusters]
